...
// find first and last digit in each line , if in line only one digit return it 2 times as first and last 


let splitInputByLines = input.split('\n')
splitInputByLines.pop()


const arrOfInputs = []


for (let i = 0; i < splitInputByLines.length; i++){

    let firstDigit 
    let lastGigit

    for(let j= 0; j <splitInputByLines[i].length; j++){
    if(!!Number(splitInputByLines[i][j])){
        if (!firstDigit) {
            firstDigit = splitInputByLines[i][j]
        }
        else {
            lastGigit = splitInputByLines[i][j]
        }
    }
    if(!lastGigit){
        lastGigit = firstDigit
    }
}
 arrOfInputs.push(Number(firstDigit + lastGigit))
}       
    const  sumOfArray = arrOfInputs.reduce((acc, current)=> acc + current, 0)
